        ...
        <div class="col-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('productos.show', $producto) }}"><i class="bi bi-cube"></i> {{ $producto->nombre }}</a></li>
            ...
            <div class="card card-outline card-warning shadow-lg">
                <div class="card-header bg-gradient-warning text-white">
                    <h3 class="card-title mb-0">
                        <i class="bi bi-pencil-square me-2"></i> Editar Producto: {{ $producto->nombre }}
                    </h3>
                    <div class="card-tools">
                        <a href="{{ route('productos.show', $producto) }}" class="btn btn-outline-light btn-sm">
                ...
                <form method="POST" action="{{ route('productos.update', $producto) }}" enctype="multipart/form-data" autocomplete="off" id="productoForm">
                    ...
                    <div class="card-body row">
                        <div class="col-md-5 text-center mb-3">
                            <label for="imagen" class="form-label fw-bold">
                                <i class="bi bi-image me-1 text-warning"></i> Imagen del Producto
                            </label>
                            <div class="form-group mb-4 text-center">
                                <label class="form-label fw-bold d-block">
                                    @if($producto->imagen)
                                        <img id="preview-imagen" src="{{ asset('storage/productos/' . $producto->imagen) }}" alt="Previsualización" class="img-fluid rounded shadow-sm mx-auto" style="max-height: 180px;">
                                        <i class="bi bi-image text-secondary fs-2" id="icono-preview" style="display: none;"></i>
                                    @else
                                        <i class="bi bi-image text-secondary fs-2" id="icono-preview" style="display: inline;"></i>
                                        <img id="preview-imagen" src="#" alt="Previsualización" class="img-fluid rounded shadow-sm d-none mx-auto" style="max-height: 180px;">
                                    @endif
                                </label>
                                <input type="file" name="imagen" id="imagen" accept="image/jpeg,image/png,image/webp" class="form-control form-control-lg @error('imagen') is-invalid @enderror">
                            @error('imagen')
                                    <div class="invalid-feedback d-block"><i class="bi bi-exclamation-triangle me-1"></i> {{ $message }}</div>
                            @enderror
                                <div class="form-text">Solo JPG, PNG o WEBP. Máx 2MB. (Opcional)</div>
                            </div>
                        </div>
                        <div class="col-md-7">
                            <div class="form-group mb-4">
                                <label for="nombre" class="form-label fw-bold">
                                    <i class="bi bi-cube me-1 text-warning"></i> Nombre <span class="text-danger">*</span>
                                </label>
                            ...
                            <div class="form-group mb-4">
                                <label for="descripcion" class="form-label fw-bold">
                                    <i class="bi bi-card-text me-1 text-warning"></i> Descripción
                                </label>
                            ...
                            <div class="form-group mb-4">
                                <label for="categoria_id" class="form-label fw-bold">
                                    <i class="bi bi-tag me-1 text-warning"></i> Categoría
                                </label>
                            ...
                            <div class="form-group mb-4">
                                <label for="stock" class="form-label fw-bold">
                                    <i class="bi bi-box-seam me-1 text-warning"></i> Stock <span class="text-danger">*</span>
                                </label>
                                <input type="number" name="stock" id="stock" class="form-control form-control-lg @error('stock') is-invalid @enderror" value="{{ old('stock', $producto->stock) }}" min="0" max="1000" required placeholder="Cantidad en stock">
                            ...
                            <div class="form-group mb-4">
                                <label for="precio" class="form-label fw-bold">
                                    <i class="bi bi-currency-dollar me-1 text-warning"></i> Precio <span class="text-danger">*</span>
                                </label>
                                <input type="number" step="0.01" name="precio" id="precio" class="form-control form-control-lg @error('precio') is-invalid @enderror" value="{{ old('precio', $producto->precio) }}" min="0.01" max="10000" required placeholder="Precio del producto">
                    ...
                    <div class="card-footer bg-light d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('productos.show', $producto) }}" class="btn btn-secondary btn-lg">
                                <i class="bi bi-x-lg me-1"></i> Cancelar
                            </a>
                            <button type="submit" class="btn btn-warning btn-lg" id="submitBtn">
                                <span class="btn-text"><i class="bi bi-check-lg me-1"></i> Actualizar Producto</span>
                                <span class="btn-loading d-none"><span class="spinner-border spinner-border-sm me-1"></span> Actualizando...</span>
                            </button>
                        </div>
                    </div>
                </form>
...
form.addEventListener('submit', function(e) {
    ...
    .then(result => {
        if (result.status === 422) {
            ...
            showToast('success', '¡Producto Actualizado!', result.data.message);
            setTimeout(() => {
                window.location.href = result.data.redirect || '{{ route("productos.index") }}';
            }, 2000);
        } else {
            showToast('error', 'Error al actualizar producto', result.data.message || 'Ha ocurrido un error inesperado.');
        }
    })
    .catch(error => {
